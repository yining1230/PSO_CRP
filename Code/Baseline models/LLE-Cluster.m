...
data = trainData;
[data1,textdata1,raw1]=xlsread('E:\博二\anli\hte_170_sigman\sigman_170.xlsx','conversion','A522:C692');
...
idx = idx(:, 2:end);
X = data;
% Compute the reconstruction weight matrix.
W = zeros(N, N);
for i = 1:N
    Xi = X(idx(i, :), :);
    Xi = Xi - repmat(X(i, :), K, 1);
    C = Xi * Xi'; 
...
% scatter(data_lle(:, 1), data_lle(:, 2), 10, 'filled');
...
centroids = zeros(length(uniqueLabels), size(data_lle, 2)); 
for i = 1:length(uniqueLabels)
clusterData = data_lle(fitnessclu == uniqueLabels(i), :); 
centroids(i, :) = mean(clusterData); 
end
C_centroids=centroids;
distances = pdist2(datap_lle, C_centroids,'seuclidean'); 

newC = knnsearch(C_centroids, datap_lle); 
disp(newC);
idx_pre=newC;
...
centroids = zeros(length(unique_labels), size(data_lle, 2)); 
...
for i = 1:length(unique_labels)
    ...
    if exist('centroid_indices', 'var')
        ...
        centroids(i, :) = data_lle(centroid_index, :);
    else
        
        cluster_data = data_lle(cluster_indices, :);
...
predicted_labels = zeros(size(datap_lle, 1), 1);

% For each new sample, compute its distance to each centroid and assign it to the nearest class.
for i = 1:size(datap_lle, 1)
    distances = pdist2(datap_lle(i, :), centroids, 'squaredeuclidean');
...
%% kmedoid
unique_labels = unique(fitnessclu);
centroids = zeros(length(fitnessclu), size(data_lle, 2)); 
...
%% spectral clustering
similarities_new = pdist2(datap_lle, data_lle,'mahalanobis');
...
centroids = zeros(length(unique_labels), size(data_lle, 2));
for i = 1:length(unique_labels)
    cluster_data = data_lle(data_lle == unique_labels(i), :);
    centroids(i, :) = mean(cluster_data);
end

predicted_labels = zeros(size(datap_lle, 1), 1);
for i = 1:size(datap_lle, 1)
    distances = pdist2(datap_lle(i, :), centroids, 'correlation');
    [~, min_distance_index] = min(distances);
    predicted_labels(i) = min_distance_index;
end
