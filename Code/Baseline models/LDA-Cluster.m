...
data = data_norm;
% Compute the class mean vectors.
class_means = zeros(max(labels), size(data, 2));
for i = 1:max(labels)
    class_means(i, :) = mean(data(labels == i, :));
end
...
for i = 1:max(labels)
    ...
    class_mean_centered = class_data - repmat(class_means(i, :), class_size, 1);
    Sw = Sw + class_mean_centered' * class_mean_centered;
end
...
for i = 1:max(labels)
    class_mean_centered = class_means(i, :) - overall_mean;
    Sb = Sb + size(data(labels == i), 1) * (class_mean_centered' * class_mean_centered);
end

[V, D] = eig(Sb, Sw);

projection_direction = V(:, 1); 

reduced_data = data * projection_direction;
data_lda = reduced_data;
...
centroids = zeros(length(uniqueLabels), size(data_lda, 2)); 
for i = 1:length(uniqueLabels)
clusterData = data_lda(fitnessclu == uniqueLabels(i), :); 
centroids(i, :) = mean(clusterData); 
end
C_centroids=centroids;
distances = pdist2(datap_lda, C_centroids,'seuclidean'); 

newC = knnsearch(C_centroids, datap_lda); 
disp(newC);
idx_pre=newC;
...
centroids = zeros(length(unique_labels), size(data_lda, 2)); 
...
for i = 1:length(unique_labels)
    ...
    if exist('centroid_indices', 'var')
        ...
        centroids(i, :) = data_lda(centroid_index, :);
    else
        
        cluster_data = data_lda(cluster_indices, :);
...
predicted_labels = zeros(size(datap_lda, 1), 1);

% For each new sample, compute its distance to each centroid and assign it to the nearest class.
for i = 1:size(datap_lda, 1)
    distances = pdist2(datap_lda(i, :), centroids, 'squaredeuclidean');
...
%% kmedoid
unique_labels = unique(fitnessclu);
centroids = zeros(length(fitnessclu), size(data_lda, 2)); 
...
%% spectral clustering
similarities_new = pdist2(datap_lda, data_lda,'mahalanobis');
...
centroids = zeros(length(unique_labels), size(data_lda, 2));
for i = 1:length(unique_labels)
    cluster_data = data_lda(data_lda == unique_labels(i), :);
    centroids(i, :) = mean(cluster_data);
end

predicted_labels = zeros(size(datap_lda, 1), 1);
for i = 1:size(datap_lda, 1)
    distances = pdist2(datap_lda(i, :), centroids, 'correlation');
    [~, min_distance_index] = min(distances);
    predicted_labels(i) = min_distance_index;
end
