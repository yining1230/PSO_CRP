...
autoencoder = trainAutoencoder(data', encoding_dim);

encoded_data = encode(autoencoder, data');
encoded_data = encoded_data';

disp(encoded_data);
data_AE = encoded_data;
%% Clusting
new_score = data_AE;
X = new_score;
k=2;
...
centroids = zeros(length(uniqueLabels), size(data_AE, 2)); 
for i = 1:length(uniqueLabels)
clusterData = data_AE(fitnessclu == uniqueLabels(i), :); 
centroids(i, :) = mean(clusterData); 
end
C_centroids=centroids;
distances = pdist2(datap_AE, C_centroids,'seuclidean'); 

newC = knnsearch(C_centroids, datap_AE); 
disp(newC);
idx_pre=newC;
...
centroids = zeros(length(unique_labels), size(data_AE, 2)); 
...
for i = 1:length(unique_labels)
    ...
    if exist('centroid_indices', 'var')
        ...
        centroids(i, :) = data_AE(centroid_index, :);
    else
        
        cluster_data = data_AE(cluster_indices, :);
...
predicted_labels = zeros(size(datap_AE, 1), 1);

% For each new sample, compute its distance to each centroid and assign it to the nearest class.
for i = 1:size(datap_AE, 1)
    distances = pdist2(datap_AE(i, :), centroids, 'squaredeuclidean');
...
%% kmedoid
unique_labels = unique(fitnessclu);
centroids = zeros(length(fitnessclu), size(data_AE, 2)); 
...
%% spectral clustering
similarities_new = pdist2(datap_AE, data_AE,'mahalanobis');
...
centroids = zeros(length(unique_labels), size(data_AE, 2));
for i = 1:length(unique_labels)
    cluster_data = data_AE(data_AE == unique_labels(i), :);
    centroids(i, :) = mean(cluster_data);
end

predicted_labels = zeros(size(datap_AE, 1), 1);
for i = 1:size(datap_AE, 1)
    distances = pdist2(datap_AE(i, :), centroids, 'correlation');
    [~, min_distance_index] = min(distances);
    predicted_labels(i) = min_distance_index;
end
