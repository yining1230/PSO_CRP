...
data_tsne = tsne(data_norm,'NumDimensions',5);
...
centroids = zeros(length(uniqueLabels), size(data_tsne, 2)); 
for i = 1:length(uniqueLabels)
clusterData = data_tsne(fitnessclu == uniqueLabels(i), :); 
centroids(i, :) = mean(clusterData); 
end
C_centroids=centroids;
distances = pdist2(datap_tsne, C_centroids,'seuclidean'); 

newC = knnsearch(C_centroids, datap_tsne); 
disp(newC);
idx_pre=newC;
...
centroids = zeros(length(unique_labels), size(data_tsne, 2)); 
...
for i = 1:length(unique_labels)
    ...
    if exist('centroid_indices', 'var')
        ...
        centroids(i, :) = data_tsne(centroid_index, :);
    else
        
        cluster_data = data_tsne(cluster_indices, :);
...
predicted_labels = zeros(size(datap_tsne, 1), 1);

% For each new sample, compute its distance to each centroid and assign it to the nearest class.
for i = 1:size(datap_tsne, 1)
    distances = pdist2(datap_tsne(i, :), centroids, 'squaredeuclidean');
...
%% kmedoid
unique_labels = unique(fitnessclu);
centroids = zeros(length(fitnessclu), size(data_tsne, 2)); 
...
%% spectral clustering
similarities_new = pdist2(datap_tsne, data_tsne,'mahalanobis');
...
centroids = zeros(length(unique_labels), size(data_tsne, 2));
for i = 1:length(unique_labels)
    cluster_data = data_tsne(data_tsne == unique_labels(i), :);
    centroids(i, :) = mean(cluster_data);
end

predicted_labels = zeros(size(datap_tsne, 1), 1);
for i = 1:size(datap_tsne, 1)
    distances = pdist2(datap_tsne(i, :), centroids, 'correlation');
    [~, min_distance_index] = min(distances);
    predicted_labels(i) = min_distance_index;
end
